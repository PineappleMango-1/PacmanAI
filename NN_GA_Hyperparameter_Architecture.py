...
#Normalizing data, not needed for this project, since all entries are either 0, 1, 2, 3, 4, or 5.
#def normalize(X):
...
#Accessing the output of the game, and setting the shape of the output of the NN --> input for the game.
game = PacmanGame()
X = one_hot(game.get_gameOutput(), 12) #Grabs the environment from the last frame and one-hot encodes it into input for the NN, with 12 possible classes for each entry.
...
#CE_loss function. Not used since it works from supervised learning --> To be found in Old_NN_GA_Architecture_No_RL.py
#class CE_loss:
...
def F(genome): #The fitness function inputs a certain amount of hyperparameters into the NN_Basic, and outputs its accuracy
    ...
    Delta_t = (t_f - t_0) * 0.01 #Penalty to equalise points with training time's influence (it should be heavily penalised if calculations take too long, since it would slow down the game significantly).
